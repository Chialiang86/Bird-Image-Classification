...
def load_raw_data(info_path, img_path, transform, class_map, aug_num=1):
    ...
    for i in range(data_size):
        print('loading {}/{}'.format(i, data_size))
...
def train(model, train_x, train_y, criterion, optimizer, device):
    ...
    for i in range(inner_num):
        optimizer.zero_grad()
        input, target = train_x[i].to(device).detach(), train_y[i].to(device).detach()
...
def val(model, val_x, val_y, criterion, optimizer, device):
    ...
    with torch.no_grad():
        ...
        for i in range(inner_num):
            optimizer.zero_grad()
            input, target = val_x[i].to(device).detach(), val_y[i].to(device).detach()
            output = model.forward(input)
...
def build_model(args, pre_trained=True):
    if args.model == 'resnet50':
        ...
        model = EfficientNet.from_pretrained(args.model, advprop=True, num_classes=classes)
        ...
        model_type = 'ViT-B_32'
        pretrained_dir = 'TransFG/pretrained/{}.npz'.format(model_type)
        config = transFG.CONFIGS[model_type]
        config.slide_step = args.bs
        model = transFG.VisionTransformer(config, img_size=320, zero_head=True, num_classes=200, smoothing_value=0.0)
...
def main(args):
    ...
    x_raw, y_raw = load_raw_data(info_path='data/training_labels.txt', img_path='data/training_images/', transform=transform_train, class_map=class_map, aug_num=aug_num)

    print('preprocessing data...')
    index_map = create_index_map(y_raw)
    train_x, train_y, val_x, val_y = train_val_split(x_raw, y_raw, index_map, 2, args.bs, aug_num)
    ...
    for epoch in range(epochs):
        ...
        train_loss, train_acc = train(model, train_x, train_y, criterion, optimizer, device=device)
        val_loss, val_acc = val(model, val_x, val_y, criterion, optimizer, device=device)
        ...
        if min_loss > val_loss and train_acc < 0.99:
            print('testing loss improved from {:.5f} to {:.5f}'.format(min_loss, val_loss))
            min_loss = val_loss
            tmp_patience = 0
        elif tmp_patience + 1 < patience_max and train_acc < 0.99:
            print('testing loss not improved from {:.5f}, got {:.5f}'.format(min_loss, val_loss))
            tmp_patience += 1
        else :
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', default='efficientnet-b3', type=str)
