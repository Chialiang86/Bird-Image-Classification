        ...
        for i in range(15):
            for cls in index_map:
                if itr + 1 < start:
                    ...
                    print('processing {}/{} -> {}'.format(itr+1, length, fnames[index_map[cls][i]]))
                    self.pos = []
                    cnt = 0
                    img = x_raw[ index_map[cls][i] ]
                    cv2.imshow('image {}'.format(fnames[index_map[cls][i]]), img)
                    cv2.setMouseCallback('image {}'.format(fnames[index_map[cls][i]]), self.__get_click_pose)
                    key = cv2.waitKey(0)
                    cv2.destroyAllWindows()
                    print(self.pos)

                    crop = img[self.pos[0][1]:self.pos[1][1], self.pos[0][0]:self.pos[1][0]]
                    cv2.imwrite('{}/{}'.format(dest, fnames[index_map[cls][i]]), crop)
                    itr += 1
    ...
    def processing_test(self, x_raw, fnames, dest, start=0):
        ...
        for i in range(length):
            if itr + 1 < start:
                ...
                print('processing {}/{} -> {}'.format(itr+1, length, fnames[i]))
                self.pos = []
                cnt = 0
                img = x_raw[i]
                cv2.imshow('image {}'.format(fnames[i]), img)
                cv2.setMouseCallback('image {}'.format(fnames[i]), self.__get_click_pose)
                key = cv2.waitKey(0)
                cv2.destroyAllWindows()
                print(self.pos)
...
def main(args):
    ...
    print('loading data ...')
    x_train, y_train, fnames_train = load_raw_data(info_path='data/training_labels.txt', img_path='data/training_images/', class_map=class_map)
    ...
    x_test = [cv2.imread('{}{}'.format(test_img_path, info)) for info in testing_info]

    cropper = Cropper()
    dest_train = 'crop/training_images'
    dest_test = 'crop/testing_images'
    print('processing ...')
    cropper.processing_train(x_train, fnames_train, index_map_train, dest_train, args.trains)
