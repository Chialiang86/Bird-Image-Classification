...
# load model
print('loading model ...')
model_path = 'result/transFG/01/bird_0.86750.pth'
...
test_x_raw = [preprocess(Image.open('{}{}'.format(test_img_path, info))) for info in testing_info]
...
for i in range(test_x.size()[0]):
    ...
    remain_bufsize = test_x_raw.size()[0] - total_cnt if total_cnt + batch_size > test_x_raw.size()[0] else batch_size
    total_cnt += remain_bufsize
    for cnt, index in enumerate(top_class[-remain_bufsize:]):
        fout.write('{} {}\n'.format(testing_info[i * batch_size + cnt], text_map[index.item()]))
